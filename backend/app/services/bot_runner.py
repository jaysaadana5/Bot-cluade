...
- Bot trades at the 4:30 mark of each window (last 30 seconds before close):
  :04:30, :09:30, :14:30, :19:30, :24:30, :29:30, etc.
"""
import asyncio
import logging
...
# Trade at 4 minutes 30 seconds into each 5-minute window
# This means 30 seconds (last 30 sec) before the window closes
TRADE_OFFSET_SECONDS = 270  # 4 * 60 + 30 = 270 seconds into the 5-min window
...
def _get_next_trade_time() -> datetime:
    ...
    Trade at 4:30 into each 5-min window (last 30 seconds before close).
    """
    now = datetime.utcnow()
...
class BotRunner:
    ...
    async def _schedule_loop(self):
        """
        Main scheduling loop - waits until the 4:30 mark of each 5-min window,
        then runs a trading cycle. This ensures trades happen in the last 30 seconds.
        """
            ...
            try:
                ...
                if wait_seconds > 0:
                    logger.info(
                        ...
                        f"({wait_seconds:.0f}s away, last 30s before window close)"
                    )
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "trade_at": "4:30 mark (last 30 sec before close)",
